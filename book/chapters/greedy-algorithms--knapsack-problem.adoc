...
> You are going to steal legumes (rice, beans, chickpeas, lentils) and you only brought a knapsack. What proportion of items can you choose to to get the highest loot without exceeding the maximum weight of the bag?
...
So, we have 4 items that we can take. We can't take them all because the total weight is `13` and the maximum we can carry is `7`. We can't just take the first one because with value `1` because obviosly is not the best profit.
