...
- *Doubly*: every node has a reference to next and previous item
...
If we
find
the element, we will return the index otherwise `undefined`. The runtime for finding an item by value is _O(n)_.
